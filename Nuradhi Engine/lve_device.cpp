...
namespace lve {
    ...
    LveDevice::LveDevice(LveWindow& window) : window{ window } {
        createInstance(); //creates vulkan instance - initializes the library & is the connection between our application and Vulkan
        setupDebugMessenger(); //validation layer - vulkan api does very little error checking. Disable this for release/performance
        createSurface(); // create surface using GLFW
        pickPhysicalDevice(); //Access GPU
        createLogicalDevice(); //describes what features of GPU to us
        createCommandPool(); //used for command buffer allocation
    }
